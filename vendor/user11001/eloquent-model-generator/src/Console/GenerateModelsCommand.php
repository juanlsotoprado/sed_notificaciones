...
class GenerateModelsCommand extends GeneratorCommand {
    ...
    private function generateEloquentModel($destinationFolder, $table, $rules) {
        ...
        $table2 = str_replace(".", "_", $table);

        //1. Determine path where the file should be generated
        $modelName = $this->generateModelNameFromTableName($table2);
    ...
    private function generateHasManyFunctions($rulesContainer) {
        ...
        foreach ($rulesContainer as $rules) {
            ...
            $hasManyModel = $this->generateModelNameFromTableName(str_replace(".", "_", $rules[0]));  //reemplazando el punto (.) y agrupando
            $key1 = $rules[1];
            $key2 = $rules[2];
            $fkName = $rules[3]; //trae el nombre del constraint

            $hasManyFunctionName = $fkName;
...
";
    ...
    private function generateHasOneFunctions($rulesContainer) {
        $functions = '';
        foreach ($rulesContainer as $rules) {
            $hasOneModel = $this->generateModelNameFromTableName(str_replace(".", "_", $rules[0]));
            $key1 = $rules[1];
            $key2 = $rules[2];
            $fkName = $rules[3];

            $hasOneFunctionName = $fkName;
...
";
    ...
    private function generateBelongsToFunctions($rulesContainer) {
        $functions = '';
        foreach ($rulesContainer as $rules) {
            $belongsToModel = $this->generateModelNameFromTableName(str_replace(".", "_", $rules[0]));
            $key1 = $rules[1];
            $key2 = $rules[2];
            $fkName = $rules[3];

            $belongsToFunctionName = $fkName;
...
";
    ...
    private function generateBelongsToManyFunctions($rulesContainer) {
        $functions = '';
        foreach ($rulesContainer as $rules) {
            $belongsToManyModel = $this->generateModelNameFromTableName(str_replace(".", "_", $rules[0]));
            $through = $rules[1];
            $key1 = $rules[2];
            $key2 = $rules[3];
            $fkName = $rules[4]; // Trae el nombre del constraint

            $belongsToManyFunctionName = $fkName;
...
";
    ...
    private function getCameCase($string) {
    ...
    private function addOneToManyRules($tables, $table, &$rules, $fk) {
        ...
        if (in_array($fkTable, $tables)) {
            $rules[$fkTable]['hasMany'][] = [$table, $field, $references, $fkName];
        }
        if (in_array($table, $tables)) {
            $rules[$table]['belongsTo'][] = [$fkTable, $field, $references, $fkName];
        }
    }
    ...
    private function addOneToOneRules($tables, $table, &$rules, $fk) {
        ...
        $references = $fk['references'];
        if (in_array($fkTable, $tables)) {
            $rules[$fkTable]['hasOne'][] = [$table, $field, $references, $fkName];
        }
        if (in_array($table, $tables)) {
            $rules[$table]['belongsTo'][] = [$fkTable, $field, $references, $fkName];
        }
    ...
    private function addManyToManyRules($tables, $table, $prep, &$rules) {
        ...
        $fk1 = $foreign[0];
        $fk1Name = $fk1['name']; /////es el nombre que se le da al constrain
        $fk1Table = $fk1['on']; ////nombre de la tabla
        $fk1Field = $fk1['field']; /////campo fk
        //$fk1References = $fk1['references']; //descomentado

        $fk2 = $foreign[1];
        $fk2Name = $fk2['name']; /////es el nombre que se le da al constrain
        $fk2Table = $fk2['on'];  ////nombre de la tabla
        $fk2Field = $fk2['field']; /////campo fk
        //$fk2References = $fk2['references']; //descomentado
        //User belongstomany groups user_group, user_id, group_id
        if (in_array($fk1Table, $tables)) {
            $rules[$fk1Table]['belongsToMany'][] = [$fk2Table, $table, $fk1Field, $fk2Field, $fk2Name]; //intercambiando el nombre $fk1Name por $fk2Name 08/11/2016
        }
        if (in_array($fk2Table, $tables)) {
            $rules[$fk2Table]['belongsToMany'][] = [$fk1Table, $table, $fk2Field, $fk1Field, $fk1Name]; //intercambiando el nombre $fk2Name por $fk1Name 08/11/2016
        }
    }
